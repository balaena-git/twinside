document.addEventListener("DOMContentLoaded", () => {
  ...
  if (avatarInput) avatarInput.addEventListener("change", () => preview(avatarInput, avatarPreview));
  if (verifyInput) verifyInput.addEventListener("change", () => preview(verifyInput, verifyPreview));
  ...
  const updateHint = () => {
    ...
    hint.innerHTML = `📓 Напишите на листике: <b>TwinSide — ${str}</b>`;
  };
  updateHint();
  ...
  if (form) form.addEventListener("submit", async (event) => {
    ...
    const selectedLooking = [...(lookingGroup?.querySelectorAll(".choice-btn.active")||[])].map((btn) => btn.dataset.value);
    const selectedInterests = [...(interestsGroup?.querySelectorAll(".choice-btn.active")||[])].map((btn) => btn.dataset.value);
    const formData = new FormData();
    if (avatarInput && avatarInput.files[0]) formData.append("avatar", avatarInput.files[0]);
    if (verifyInput && verifyInput.files[0]) formData.append("verify_photo", verifyInput.files[0]);
    formData.append("about", document.getElementById("about")?.value || "");
    formData.append("looking_for", selectedLooking.join(","));
    formData.append("interests", selectedInterests.join(","));
    try {
      const data = await PublicApp.request("/profile/setup", { method: "POST", body: formData });
      if (data.ok) {
